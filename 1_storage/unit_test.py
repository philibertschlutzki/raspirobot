...
"""
Umfassendes Testscript für Path Recording System
=================================================
Testet alle Funktionen von:
- path_recording_system_fixed.py (Datenstrukturen)
...
from path_recording_system_fixed import (
    ControllerSample,
    LidarFrame,
    Checkpoint,
    CalibrationData,
...
class TestPathRecordingStorageManager(unittest.TestCase):
    ...
    def test_save_auto(self):
        """Test: Automatische Format-Wahl basierend auf Dateierweiterung"""
        # JSON
        json_path = self.storage_manager.save_auto(
            self.test_recording,
            filename="test_auto.json.gz"
        )
        self.assertTrue(json_path.endswith(".json.gz"))
        
        # Binary
        binary_path = self.storage_manager.save_auto(
            self.test_recording,
            filename="test_auto.prec"
        )
        self.assertTrue(binary_path.endswith(".prec"))
        
    def test_load_auto(self):
        """Test: Automatisches Laden basierend auf Dateierweiterung"""
        json_path = self.storage_manager.save_json(
            self.test_recording,
            filename="test_load_auto_json"
        )
        binary_path = self.storage_manager.save_binary(
            self.test_recording,
            filename="test_load_auto_binary"
        )
        
        loaded_json = self.storage_manager.load_auto(json_path)
        loaded_binary = self.storage_manager.load_auto(binary_path)
        
        self.assertEqual(len(loaded_json.controller_samples), 5)
        self.assertEqual(len(loaded_binary.controller_samples), 5)
    ...
    def test_backup_functionality(self):
        ...
        # Überschreibe Datei (sollte Backup erstellen)
        modified_recording = self._create_test_recording()
        self.storage_manager.save_json(
            modified_recording,
            filename="test_backup"
        )
        ...
        backups = list(backup_dir.glob("test_backup_backup_*"))
    ...
    def test_validate_nonexistent_file(self):
        """Test: Validierung nicht-existierender Datei"""
        validation = self.storage_manager.validate_file("nonexistent.json")
    ...
    def test_corrupted_file_handling(self):
        """Test: Umgang mit korrupten Dateien"""
        # Erstelle korrupte Datei
        corrupt_path = Path(self.temp_dir) / "corrupt.json.gz"
        with open(corrupt_path, 'wb') as f:
            f.write(b'CORRUPTED DATA')
            
        validation = self.storage_manager.validate_file(corrupt_path)
        self.assertFalse(validation['valid'])
        self.assertGreater(len(validation['errors']), 0)
...
class TestIntegration(unittest.TestCase):
    ...
    def test_complete_recording_workflow(self):
        ...
        for i in range(100):  # 100 Controller Samples (2 Sekunden @ 50Hz)
            sample = ControllerSample(
        ...
        for i in range(10):  # 10 LIDAR Frames (2 Sekunden @ 5Hz)
            scan_data = [
        ...
        checkpoint = Checkpoint(
            ...
            notes="Mittelpunkt der Aufzeichnung"
        )
        ...
        # 5. Speichere in verschiedenen Formaten
        ...
        # 6. Validiere gespeicherte Dateien
        ...
        self.assertEqual(len(loaded_binary.lidar_frames), 10)
        self.assertEqual(len(loaded_json.checkpoints), 1)
        
        # 8. Statistiken prüfen
        stats = loaded_json.get_stats()
        self.assertEqual(stats['controller_samples'], 100)
        self.assertEqual(stats['lidar_frames'], 10)
        self.assertEqual(stats['checkpoints'], 1)
        self.assertGreater(stats['duration_seconds'], 0)
        
    def test_multi_session_management(self):
        """Test: Verwaltung mehrerer Sessions"""
        ...
        for session_num in range(3):
            ...
            self.storage_manager.save_json(recording, f"session_{session_num:03d}")
            
        # Liste alle Recordings
        recordings = self.storage_manager.list_recordings()
        self.assertGreaterEqual(len(recordings), 3)
        
        # Validiere alle
        for rec_info in recordings:
            validation = self.storage_manager.validate_file(rec_info['filepath'])
            self.assertTrue(validation['valid'])
...
def run_all_tests():
    ...
    print("=" * 80)
    print("PATH RECORDING SYSTEM - UMFASSENDE TESTSUITE")
    print("=" * 80)
    print()
