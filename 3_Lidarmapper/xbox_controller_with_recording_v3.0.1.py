...
NEU in Version 3.0.1 (Bugfix Release):
- ✅ Race Condition bei LIDAR-Beendigung behoben
- ✅ LIDAR-Frame-Buffer Flush implementiert
- ✅ Verbesserte Thread-Synchronisation
- ✅ Robustere Cleanup-Sequenz
...
AUTOR: Phase 3 Integration (Bugfix v3.0.1)
DATUM: 2025-11-27
VERSION: 3.0.1 (LIDAR Beendigung Fix)
"""
...
# Thread Control
running = False
shutdown_event = threading.Event()  # NEU: Event für koordiniertes Shutdown
...
recording_lock = threading.Lock()  # NEU: Lock für recording_active
...
def lidar_update_thread():
    """Thread für LIDAR-Scan-Verarbeitung mit verbesserter Shutdown-Logik."""
    ...
    try:
        for scan in lidar.iter_scans(max_buf_meas=5000, min_len=10):
            # VERBESSERT: Prüfe beide Shutdown-Flags
            if not running or shutdown_event.is_set():
                print('[THREAD] LIDAR Thread: Shutdown erkannt')
                break
            ...
            try:
                ...
                # VERBESSERT: Prüfe recording_active thread-sicher
                with recording_lock:
        ...
        print(f'[THREAD] LIDAR Thread beendet ({scan_count} Scans verarbeitet)')
...
# ============================================================================
# RECORDING FUNKTIONEN (VERBESSERT)
# ============================================================================
...
def create_recording_data() -> PathRecordingData:
    ...
    metadata = EnvironmentMetadata(
        ...
        environment_conditions={
            ...
            'robot_configuration': 'xbox_controller_lidar_v3.0.1'
        },
        hardware_info={
...
def start_recording() -> bool:
    ...
    with recording_lock:
        ...
        recording_active = True  # Setze Flag unter Lock
...
def stop_recording(force_save: bool = False) -> bool:
    ...
    VERBESSERT: Flush LIDAR-Frame-Buffer vor dem Speichern.
    """
    ...
    with recording_lock:
        ...
        # WICHTIG: Setze Flag sofort, damit LIDAR-Thread keine neuen Frames schreibt
        recording_active = False
    
    try:
        print('[RECORDING] ⏹️ Stoppe Recording...')
        
        # NEU: Warte kurz, damit laufende Buffer-Writes abgeschlossen werden
        time.sleep(0.2)
        
        # NEU: Flush alle verbleibenden LIDAR-Frames aus Buffer
        frames_flushed = 0
        flush_timeout = time.time() + 2.0  # Max 2 Sekunden für Flush
        
        print('[RECORDING] 📦 Flush LIDAR-Frame-Buffer...')
        while time.time() < flush_timeout:
            try:
        ...
        if frames_flushed > 0:
            print(f'[RECORDING] ✅ {frames_flushed} LIDAR-Frames nachträglich gespeichert')
        ...
        if is_valid and recording_data and storage_manager:
            ...
            if logger:
                logger.info(f"Recording gespeichert: {filepath} (Controller: {controller_samples}, LIDAR: {lidar_frames})")
...
def add_data_to_recording():
    ...
    # Thread-sichere Prüfung
    with recording_lock:
        is_recording = recording_active
    
    if not is_recording or not recording_data:
        return
    ...
    try:
        ...
        # LIDAR Frames aus Buffer (max 5 pro Cycle)
        frames_added = 0
...
def recording_background_thread():
    ...
    print('[THREAD] Recording Thread beendet')
...
def led_control_thread():
    ...
    # Cleanup: LED ausschalten
    GPIO.output(STATUS_LED_PIN, GPIO.LOW)
    print('[THREAD] LED Thread beendet')
...
def cleanup():
    """
    Ressourcen-Freigabe mit verbesserter LIDAR-Beendigung.
    """
    ...
    running = False
    shutdown_event.set()  # Signalisiere allen Threads
    
    # 1. Stoppe Recording mit vollständigem Buffer-Flush
    with recording_lock:
        is_recording = recording_active
    
    if is_recording:
        print('[CLEANUP] 💾 Sichere Recording mit vollständigem Buffer-Flush...')
        stop_recording(force_save=True)
    
    # 2. Warte auf Thread-Beendigung mit Timeouts
    threads_to_join = [
    ...
    for name, thread in threads_to_join:
        if thread and thread.is_alive():
            ...
            thread.join(timeout=THREAD_SHUTDOWN_TIMEOUT_SEC)
            if thread.is_alive():
                print(f'[CLEANUP] ⚠️ {name} Thread Timeout - fortfahren...')
    
    # 3. Stoppe Motoren
    print('[CLEANUP] Stoppe Motoren...')
    if pwm_left:
        try:
            pwm_left.change_duty_cycle(MIN_DUTY_CYCLE)
            pwm_left.stop()
        except Exception as e:
            print(f'[CLEANUP] PWM Left Fehler: {e}')
    ...
    if pwm_right:
        try:
            ...
            print(f'[CLEANUP] PWM Right Fehler: {e}')
    
    # 4. VERBESSERT: Stoppe LIDAR mit korrekter Sequenz
    if lidar and lidar_connected:
        try:
            print('[CLEANUP] Stoppe LIDAR Scan...')
            lidar.stop()
            
            # WICHTIG: Warte auf vollständigen Scan-Stop
            print(f'[CLEANUP] Warte {LIDAR_STOP_DELAY_SEC}s auf Scan-Stop...')
            time.sleep(LIDAR_STOP_DELAY_SEC)
            
            print('[CLEANUP] Stoppe LIDAR Motor...')
            lidar.stop_motor()
            
            # WICHTIG: Warte auf vollständigen Motor-Stop
            print(f'[CLEANUP] Warte {LIDAR_MOTOR_STOP_DELAY_SEC}s auf Motor-Stop...')
            time.sleep(LIDAR_MOTOR_STOP_DELAY_SEC)
            
            print('[CLEANUP] Trenne LIDAR...')
            lidar.disconnect()
            
            print('[CLEANUP] ✅ LIDAR korrekt beendet')
            
        except Exception as e:
            print(f'[CLEANUP] ⚠️ LIDAR Stop-Fehler: {e}')
    ...
    try:
        GPIO.cleanup()
        print('[CLEANUP] GPIO bereinigt')
    except Exception as e:
        print(f'[CLEANUP] GPIO Cleanup Fehler: {e}')
...
def main():
    ...
    print('Version 3.0.1 - Bugfix Release (LIDAR Beendigung)')
    print('='*80 + '\n')
...
if __name__ == '__main__':
    ...
    if os.geteuid() != 0:
        ...
        print('📝 Verwendung: sudo python3 xbox_controller_with_recording_v3.0.py')
        sys.exit(1)
