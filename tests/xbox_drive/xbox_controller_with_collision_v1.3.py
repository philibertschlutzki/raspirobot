...
- Wenn A-Button gehalten: Rechter Trigger vorwärts → Linker Motor rückwärts (Drehung nach links)
- Wenn A-Button gehalten: Linker Trigger vorwärts → Rechter Motor rückwärts (Drehung nach rechts)
- Ohne A-Button: Normales Fahrverhalten wie in Version 1.2
...
Tank-Turn Modus (NEU in v1.3):
- A-Button NICHT gedrückt: Normales Fahren (beide Trigger steuern jeweiligen Motor vorwärts/rückwärts)
- A-Button GEHALTEN:
  * Rechter Trigger vorwärts → Rechter Motor vorwärts, Linker Motor RÜCKWÄRTS (Drehung nach links)
  * Linker Trigger vorwärts  → Linker Motor vorwärts, Rechter Motor RÜCKWÄRTS (Drehung nach rechts)
  * Beide Trigger → Roboter dreht an Ort (Richtung abhängig von stärkerem Trigger)
...
7. Tank-Turn Modus visuell angezeigt

VERWENDUNG:
----------
sudo python3 xbox_controller_with_collision_v1.3.py

Beenden: Ctrl+C oder Start-Button am Controller

AUTOR: Automatisch generiert und erweitert
DATUM: 2025-10-10
VERSION: 1.3 (A-Button Tank-Turn implementiert)
"""
...
# Richtungslogik
DIR_HIGH_IS_BACKWARD = True  # True => HIGH = rückwärts, LOW = vorwärts
...
BUTTON_A = 0                 # A-Button für Tank-Turn Modus (NEU!)
...
trigger_right = -1.0
a_button_pressed = False    # Tank-Turn Modus (NEU!)
...
def setup_gpio():
    ...
    # Anfangsrichtung auf "vorwärts" setzen
...
def set_motor_direction(left_backward: bool, right_backward: bool):
    ...
    Note:
        Diese Funktion ersetzt set_direction() und erlaubt individuelle
        Motorrichtungen für Tank-Turn Funktion.
    """
    if DIR_HIGH_IS_BACKWARD:
...
def read_controller_input():
    ...
    NEU in v1.3:
    - A-Button Status wird gelesen für Tank-Turn Modus
    """
...
def control_loop():
    ...
    NEU in v1.3:
    - Tank-Turn Logik implementiert mit A-Button
    - Individuelle Motorrichtungssteuerung je nach Modus
    """
    ...
    while running:
        ...
        # 3. Tank-Turn Logik (NEU in v1.3!)
        if a_button_pressed:
            # A-Button gehalten: Tank-Turn Modus aktiviert
            # Bestimme welche Motoren in welche Richtung drehen

            if is_moving_forward:
                # Normaler Vorwärts-Modus mit Tank-Turn
                # Rechter Trigger vorwärts → Rechter Motor vor, Linker rück (Links-Drehung)
                # Linker Trigger vorwärts → Linker Motor vor, Rechter rück (Rechts-Drehung)

                # Setze Motorrichtungen für Tank-Turn
                # Rechter Motor: vorwärts wenn Right Trigger, rückwärts wenn Left Trigger
                # Linker Motor: vorwärts wenn Left Trigger, rückwärts wenn Right Trigger
                right_motor_backward = (left_duty > MIN_DUTY_CYCLE)
                left_motor_backward = (right_duty > MIN_DUTY_CYCLE)

                set_motor_direction(left_motor_backward, right_motor_backward)
            else:
                # Rückwärts-Modus mit Tank-Turn (Logik invertiert)
                right_motor_backward = not (left_duty > MIN_DUTY_CYCLE)
                left_motor_backward = not (right_duty > MIN_DUTY_CYCLE)

                set_motor_direction(left_motor_backward, right_motor_backward)
        else:
            # A-Button NICHT gedrückt: Normales Fahren (beide Motoren gleiche Richtung)
            backward = not is_moving_forward
            set_motor_direction(backward, backward)

        # 4. Setze Motor Geschwindigkeit (mit Kollisionsvermeidung)
        set_motor_speed(left_duty, right_duty)

        # 5. Status Ausgabe (jede Sekunde)
        status_counter += 1
...
def main():
    ...
    print('Version 1.3 - 2025-10-10 (A-Button Tank-Turn implementiert)')
    print('='*80 + '\n')
...
if __name__ == '__main__':
    ...
    if os.geteuid() != 0:
        ...
        print('📝 Verwendung: sudo python3 xbox_controller_with_collision_v1.3.py')
        sys.exit(1)
